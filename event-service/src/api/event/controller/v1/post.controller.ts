...
import {CreatePostResDto, DeletePostResDto, ListPostResDto, UpdatePostResDto} from '../../dto/post-res.dto';
...
export class PostsController {
    ...
    async list(@Query('event_id') eventId: number, @Query('page') page: number, @Req() req: any):
        Promise<ResponseDto> {
        let limit = 20;
        page = page ? page : 1;
    ...
    async update(@Body() dto: UpdatePostReqDto, @Param('post_id') postId: number, @Req() req: any):
        Promise<ResponseDto> {
